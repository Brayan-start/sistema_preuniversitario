...
class MetricsRepository
{
    public function totals(): array
    {
        return [
            ...
            'total_pagos_realizados' => Pago::where('estado', 'aprobado')->count(),
            'total_pagos_pendientes' => Pago::whereIn('estado', ['pendiente', 'en_revision'])->count(),
            'ingresos_acumulados' => (float) Pago::where('estado', 'aprobado')->sum('monto'),
            'ingresos_mes_actual' => (float) Pago::where('estado', 'aprobado')
    ...
    {
        return $this->buildMonthlyCountSeries(Inscripcion::query(), $range, 'created_at');
    }

    public function monthlyIngresos(DateRangeFilterData $range): array
    {
        return $this->buildMonthlySumSeries(Pago::query()->where('estado', 'aprobado'), $range, 'fecha_pago', 'monto');
    }
    ...
    {
        $query = Inscripcion::query()->with('curso');
        $this->applyRange($query, $range, 'created_at');
    ...
    {
        return Inscripcion::with(['aspirante', 'curso'])
            ->latest()
            ->limit($limit)
            ->get();
    }
    ...
    private function percentageCompletedPayments(): float
    {
        ...
        return round((Pago::where('estado', 'aprobado')->count() / $total) * 100, 2);
    }
    ...
    private function averageInscripcionesByCurso(): float
    {
        ...
        return round(Inscripcion::count() / $activeCourses, 2);
    }
