                ...
                @if($inscripcion)
                    ...
                    <div class="mb-3">
                        ...
                        @php
                            ...
                            $statusLabel = match($inscripcion->estado) {
                                'aprobado' => 'APROBADO',
                                'rechazado' => 'RECHAZADO',
                    ...
                    @if($inscripcion->estado === 'aprobado')
                        <div class="alert alert-success border-0 rounded-4 py-3 px-3 mb-3">
                            <i class="fas fa-check-circle me-1"></i>
                            <strong>¡Felicidades!</strong> Tu inscripción fue aprobada.
                        </div>
                        @if($inscripcion->grupo)
                        <div class="mb-3">
                            <label class="small text-muted d-block">Paralelo / Grupo</label>
                            <span class="fw-bold fs-5 text-dark">{{ $inscripcion->grupo }}</span>
                        </div>
                        @endif
                    @elseif($inscripcion->estado === 'rechazado')
                        <div class="alert alert-danger border-0 rounded-4 py-3 px-3 mb-3">
                            <i class="fas fa-times-circle me-1"></i>
                            <strong>Inscripción rechazada.</strong>
                        </div>
                        @if($inscripcion->motivo_rechazo)
                        <div class="mb-3">
                            <label class="small text-muted d-block">Motivo del rechazo</label>
                            <p class="mb-0 fw-medium text-danger">{{ $inscripcion->motivo_rechazo }}</p>
                        </div>
                        @endif
                    @elseif($inscripcion->estado === 'en_revision')
                        <div class="alert alert-warning border-0 rounded-4 py-3 px-3 mb-3">
                            <i class="fas fa-clock me-1"></i>
                            <strong>En revisión.</strong> Pronto recibirás una notificación.
                        </div>
                    @else
                        <div class="alert alert-secondary border-0 rounded-4 py-3 px-3 mb-3">
                            <i class="fas fa-hourglass-half me-1"></i>
                            <strong>Pendiente.</strong> Completa los pasos indicados.
                        </div>
                    @endif
                    ...
                    @if($inscripcion->fecha_cambio_estado)
                    <div class="mb-0">
                        <label class="small text-muted d-block">Última actualización</label>
                        <span class="fw-medium">{{ $inscripcion->fecha_cambio_estado->format('d/m/Y H:i') }}</span>
                    </div>
                    @endif
                @else
            ...
            <div class="card-body p-0">
                <div class="list-group list-group-flush">
                    ...
                    @else
                        ...
                        <div class="list-group-item p-4">
                            <div class="d-flex align-items-center">
                                <div class="rounded-circle d-flex align-items-center justify-content-center me-4 {{ $inscripcion->estado === 'aprobado' ? 'bg-success text-white' : ($inscripcion->estado === 'rechazado' ? 'bg-danger text-white' : 'bg-light text-muted border') }}" style="width: 45px; height: 45px; flex-shrink: 0;">
                                    @if($inscripcion->estado === 'aprobado')
                                ...
                                <div class="flex-grow-1">
                                    @if($inscripcion->estado === 'aprobado')
                                        <h6 class="fw-bold mb-1 text-success">¡Inscripción Aprobada!</h6>
                                        <p class="text-muted small mb-0">
                                            <strong>Paralelo:</strong> {{ $inscripcion->grupo ?? 'Por asignar' }}.
                                        </p>
                                    @elseif($inscripcion->estado === 'rechazado')
                                        <h6 class="fw-bold mb-1 text-danger">Inscripción Rechazada</h6>
                                        <p class="text-muted small mb-0">
                                            Motivo: {{ $inscripcion->motivo_rechazo ?? 'No especificado' }}.
                                        </p>
                                    @else
